...
class Compose(object):
    ...
    def __call__(self, img, mask, depth):
        img, mask,depth = Image.fromarray(img, mode="RGB"), Image.fromarray(mask, mode="L"), Image.fromarray(depth.astype(np.float32), mode='F')
        assert img.size == mask.size
        assert img.size == depth.size
        for a in self.augmentations:
            img, mask, depth = a(img, mask, depth)
        return np.array(img), np.array(mask, dtype=np.uint8), np.array(depth, dtype=np.float32)
...
class RandomCrop_city(object):  # used for results in the CVPR-19 submission
    ...
    def __call__(self, img, mask, depth):
        ...
        depth = depth.resize((int(w/2), int(h/2)), Image.NEAREST)
...
class RandomCrop_city_gnet(object):  # used for gnet training
    ...
    def __call__(self, img, mask, depth):
        ...
        img = img.resize((600, 300), Image.BILINEAR)
        mask = mask.resize((600, 300), Image.NEAREST)
        depth = depth.resize((600, 300), Image.NEAREST)
        #img = img.resize((512, 256), Image.BILINEAR)
        #mask = mask.resize((512, 256), Image.NEAREST)
...
class RSCrop_city(object):
    ...
    def __call__(self, img, mask, depth):
        ...
        img = img.rotate(deg, resample=Image.BILINEAR)
        mask = mask.rotate(deg, resample=Image.NEAREST)
        depth = depth.rotate(deg, resample=Image.NEAREST)
        ...
        depth = depth.crop((x1, y1, x1+self.size[0], y1+self.size[1]))
    ...
    def __call__(self, img, mask, depth):
        ...
        for attempt in range(10):
            ...
            if w <= img.size[0] and h <= img.size[1]:
                ...
                depth = depth.crop((x1, y1, x1 + w, y1 + h))
                assert img.size == (w, h)

                return (
                    img.resize((f_w, f_h), Image.BILINEAR),
                    mask.resize((f_w, f_h), Image.NEAREST),
                    depth.resize((f_w, f_h), Image.NEAREST),
                )

        # Fallback
        scale = Scale(self.size)
        crop = CenterCrop(self.size)
        return crop(*scale(img, mask, depth))
...
class RandomSized(object):
    ...
    def __call__(self, img, mask, depth):
        ...
        return self.crop(*self.scale(img, mask, depth))
